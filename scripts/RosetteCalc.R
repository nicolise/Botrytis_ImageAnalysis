...
if(length(RawFiles) == length(LeafMaskFiles)  {
  ...
  rm(ImageHSV)
    ...
    for(i in 1:max(LeafMask.lab)) {
      ...
      MoreResults[i,] <- c(Leaf.Size, Leaf.Grn, Leaf.Ylw)
      
    }
    
    colnames(MoreResults) <- c("Leaf.Size", "Leaf.Grn", "Leaf.Ylw")
